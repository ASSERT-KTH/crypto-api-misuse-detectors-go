...
func NewModComposer(ds *dataset.ModuleDataset, cc ComposerConfig) *ModComposer {
	return &ModComposer{
		Dataset: ds,
		config:  cc,
	}
...
func (mc *ModComposer) GetConfig() ComposerConfig {
	return mc.config
}
...
func (mc *ModComposer) generateModServices(mod dataset.Module) string {
	...
	metadataWriter := dataset.NewMetadataWriter(mc.config.ResultsDir)
