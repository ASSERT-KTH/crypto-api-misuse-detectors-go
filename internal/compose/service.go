...
type Service struct {
	...
	Config ComposerConfig // Configuration from the composer
}
...
type ServiceBuilder struct {
	...
	Config ComposerConfig // Configuration including results directory structure
}

// NewServiceBuilder creates a new ServiceBuilder with the given tools
func NewServiceBuilder(tools []tool.Tool, config ComposerConfig) *ServiceBuilder {
...
func NewService(repoID, repoURL, gitTag, goVersion string, tool tool.Tool, cfg ComposerConfig) (Service, error) {
	svcName := fmt.Sprintf("%s-%s", repoID, tool.Name())

	// Create path configuration for this service
	paths := NewServicePaths(cfg.ResultsDir, repoID, tool.Name())
...
func (s *Service) GenerateStr() string {
	...
	buf.WriteString(s.generateEnvSpec(toolCfg)) // TODO fix this encapsulation
...
func (s *Service) generateServiceConfig() string {
	...
	buf.WriteString("    cpus: 2\n")
	buf.WriteString(fmt.Sprintf("    mem_limit: %s\n", s.Config.Memory.Limit))
	buf.WriteString(fmt.Sprintf("    memswap_limit: %s\n", s.Config.Memory.SwapLimit))
	return buf.String()
}
...
func (s *Service) generateEnvSpec(toolCfg tool.DockerConfig) string {
	var buf strings.Builder
	if toolCfg.Environment != nil {
		buf.WriteString("    environment:\n")
		for _, env := range toolCfg.Environment {
			buf.WriteString(fmt.Sprintf("      - %s\n", env))
		}
	}
	return buf.String()
